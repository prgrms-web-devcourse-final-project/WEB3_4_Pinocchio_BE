...
const StyledFilePond = styled(FilePond)`
  ...
  .filepond--item {
    ...
    height: auto !important; /* 높이를 자동 조절 */
    gap: 10px;
    overflow: hidden;
  }
  /* 이미지 프리뷰 컨테이너 */
  ...
  .filepond--image-preview {
    width: 100%;
    height: 150px !important;
    object-fit: contain !important; /* 이미지를 크기에 맞게 조정 */
    background-color: white !important;
    overflow: hidden !important;
  }
  ...
  .filepond--image-preview-overlay {
    ...
    display: none !important; /* 필요하면 숨김 처리 */
  }
  .filepond--image-preview-overlay-idle,
  .filepond--image-preview-overlay-success,
  .filepond--image-preview-overlay-failure {
    background-color: transparent !important; /* 완전히 투명하게 설정 */
...
/*
  껍데기가 있으면서 이미지 뷰어 기능이 있는 파일 업로드 컴포넌트 입니다.
  껍데기가 있다 = 게시물을 만들 때에 파일 업로드 없이 먼저 게시물 생성 -> 해당 게시물의 파일 추가 의 개념입니다.
  ex) 요청, 게시판 수정
*/
const YesMappingFileWithImageViewer = (
    {
        label,            // 파일 업로드 안내 문구
    ...
    const fetchExistingFiles = async () => {
        try {
            ...
            if (existingFiles && existingFiles.length > 0) {
                const filePondFiles = await Promise.all(existingFiles.map(async (file) => {
                    const isUserFile = file.createUserId === userId;

                    // 파일을 다운로드하여 Blob 변환
                    const fileResponse = await axios.get(`/api/upload/${moduleType}/${moduleItemId}/file/${file.uploadId}`, {
                        responseType: 'blob',
                    });
                    ...
                    if (mimeType.startsWith("image")) {
                        // 이미지 파일인 경우 그대로 데이터 세팅
                        fileBlob = new File([fileResponse.data], file.originFileName, { type: mimeType });
                        fileUrl = URL.createObjectURL(fileBlob);
                        previewUrl = fileUrl;
                    } else if (mimeType.startsWith("application/pdf")){
                        // fileBlob에 기본 파일 이미지의 blob 데이터 세팅
                        const fileImageResponse = await fetch(defaultPdf);
                        const fileImageBlob = await fileImageResponse.blob();
                        fileBlob = new File([fileImageBlob], file.originFileName, { type: "image/png" });

                        // fileUrl에 실제 pdf파일의 blob 데이터 세팅
                        const originFileBlob = new Blob([fileResponse.data], { type: "application/pdf" });
                        fileUrl = URL.createObjectURL(originFileBlob)
                        previewUrl = fileUrl;
                    } else {
                        // 이미지 또는 pdf 파일이 아닌 경우 기본 파일 이미지 데이터 세팅
                        const fileImageResponse = await fetch(defaultFile);
                        const fileImageBlob = await fileImageResponse.blob();
                        fileBlob = new File([fileImageBlob], file.originFileName, { type: "image/png" });
                        fileUrl = URL.createObjectURL(fileBlob);
                        previewUrl = fileUrl;
                    ...
                    return {
                        source: previewUrl, // 여기서 object URL을 source로 사용
                        options: {
                            type: 'local',
                            file: fileBlob
                        },
                        metadata: {
                            ...
                            uploadDate: dateFormat(file.createTime, "yyyy-MM-dd a/p hh:mm"),
                ...
                // 이미지 파일만 따로 필터링
                const slides = filePondFiles
                    .filter(file => file.metadata.originalMimeType.startsWith("image"))
                    .map(file => ({
                        src: file.source,
    ...
    useEffect(() => {
        if (moduleType && moduleItemId && userId && userId !== 0) {
            fetchExistingFiles();
        }
    ...
    return (
        <>
            ...
            파일을 이곳에 드래그 & 드롭하거나<br>
           <span class="filepond--label-action">찾아보기</span> 버튼을 클릭하세요<br>
           <span class="text-orange">(최대 ${maxFiles}개 업로드 가능)
           </div>`}
                allowMultiple={true}
                maxFiles={maxFiles}
                allowPaste={false}
                ...
                onactivatefile={(file) => {
                    ...
                    const matchingFile = files.find(f => f.options?.file?.name === clickedFileName);

                    if (matchingFile?.metadata) {
                        const { uploadId, originalMimeType, fileUrl  } = matchingFile.metadata;
                        if (originalMimeType?.startsWith("image")) {
                            // 이미지 파일이면 뷰어 열기
                            const index = imageSlideData.findIndex(image => image.uploadId === uploadId);
                            if (index !== -1) {
                                setCurrentIndex(index);
                                setShowImageViewer(true);
                            }
                        } else if (originalMimeType?.startsWith("application/pdf")) {
                            // pdf 파일인 경우 새 창에 pdf 화면 열기
...
export default YesMappingFileWithImageViewer