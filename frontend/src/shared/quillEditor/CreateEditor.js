...
const CreateEditor = (
    ...
    // 툴바 아이콘
    const formats = ['header', 'bold', 'italic', 'underline', 'strike', 'list', 'bullet', 'indent', 'color', 'link', 'image',];
    // 툴바 높이를 저장할 상태
    ...
    useEffect(() => {
        if (quillRef.current) {
            ...
            const toolbarHeight = toolbarElement.clientHeight || 43;  // 툴바의 실제 높이 가져오기
    ...
    const handleFileUpload = async (file) => {
        ...
        try {
            ...
            const uniqueFileId = response.data[0].uploadTempId;
            return uniqueFileId
        } catch (err) {
            console.log(err);
            /*Alert({
    ...
    useEffect(() => {
        if (mentionBoxVisible && mentionBoxRef.current && mentionTriggerBounds) {
            ...
            const editorContainer = quillRef.current.getEditor().root.parentNode;
