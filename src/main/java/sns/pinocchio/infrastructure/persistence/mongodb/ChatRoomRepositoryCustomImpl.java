...
public class ChatRoomRepositoryCustomImpl implements ChatRoomRepositoryCustom {
  ...
  @Override
  public List<Chat> findChatsByChatRoomWithCursor(
      String chatRoomTsid, String cursor, int limit, ChatRoomSortType sortType) {

    Query query = new Query();

    // 해당 유저가 포함된 채팅방
    query.addCriteria(Criteria.where("roomTsid").is(chatRoomTsid));

    // 커서가 있을 경우, 생성 시간 기준으로 이전 것만 가져오기
    if (cursor != null) {
      query.addCriteria(Criteria.where("createdAtForTsid").lt(cursor));
    }

    // 정렬 타입에 맞게, 생성 시간 기준으로 정렬
    if (sortType == ChatRoomSortType.LATEST) {
      query.with(Sort.by(Sort.Direction.DESC, "createdAtForTsid"));

    } else {
      query.with(Sort.by(Sort.Direction.ASC, "createdAtForTsid"));
    }

    // 제한 개수 설정
    query.limit(limit);

    return mongoTemplate.find(query, Chat.class, "chats");
  }
}
